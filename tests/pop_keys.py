from index_by import index_by_pop_key, index_by_pop_key_as_list, index_by_pop_keys, index_by_pop_keys_as_list
import unittest
...
class TestIndexByKey(unittest.TestCase):
    def test_key_found(self):
        ...
        index = index_by_pop_key(obj_list, 'key')
    ...
    def test_key_not_found(self):
        ...
        with self.assertRaises(KeyError):
            index_by_pop_key(obj_list, 'non_existing_key')

    def test_key_is_popped(self):
        obj_list = [dict(key=0), dict(key=0), dict(key=0)]
        index_by_pop_key(obj_list, 'key')
        for obj in obj_list:
...
class TestIndexByKeyAsList(unittest.TestCase):
    def test_key_found(self):
        ...
        index = index_by_pop_key_as_list(obj_list, 'key')
        self.assertEqual(index[0], obj_list[0:2])
        self.assertEqual(index[1], obj_list[2:])

    def test_key_is_popped(self):
        obj_list = [dict(key=0), dict(key=0), dict(key=0)]
        index_by_pop_key_as_list(obj_list, 'key')
        for obj in obj_list:
            self.assertNotIn('key', obj)
...
class TestIndexByKeys(unittest.TestCase):
    def test_key_found(self):
        ...
        index = index_by_pop_keys(obj_list, ('compound_key_one', 'compound_key_two'))
    ...
    def test_key_not_found_full(self):
        ...
        with self.assertRaises(KeyError):
            index_by_pop_keys(obj_list, ('non_existing_key_one', 'non_existing_key_two'))

    def test_key_not_found_partial(self):
        obj_list = [dict(compound_key_one=0, compound_key_two=0)]
        with self.assertRaises(KeyError):
            index_by_pop_keys(obj_list, ('compound_key_one', 'non_existing_key_two'))

    def test_key_is_popped(self):
        obj_list = [dict(compound_key_one=0, compound_key_two=0), dict(compound_key_one=0, compound_key_two=0)]
        index_by_pop_keys(obj_list, ('compound_key_one', 'compound_key_two'))
        for obj in obj_list:
            self.assertNotIn('compound_key_one', obj)
            self.assertNotIn('compound_key_two', obj)
...
class TestIndexByKeysAsList(unittest.TestCase):
    def test_key_found(self):
        ...
        index = index_by_pop_keys_as_list(obj_list, ('compound_key_one', 'compound_key_two'))
        self.assertEqual(index[(0, 0)], obj_list[0:2])
        self.assertEqual(index[(1, 0)], obj_list[2:])

    def test_key_is_popped(self):
        obj_list = [dict(compound_key_one=0, compound_key_two=0), dict(compound_key_one=0, compound_key_two=0)]
        index_by_pop_keys_as_list(obj_list, ('compound_key_one', 'compound_key_two'))
        for obj in obj_list:
            self.assertNotIn('compound_key_one', obj)
            self.assertNotIn('compound_key_two', obj)
